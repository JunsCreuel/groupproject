...
import { SongPlayer, getAudioCtx } from './audioEngine.js';
...
// .receptor { top: 78% })다. 이 값은 반드시 index.css의 .receptor top%와
// 같은 값으로 맞춰야 한다.
const RECEPTOR_TOP_PERCENT = 78;

export default function Game({ difficulty, paused, onFinish }) {
  ...
  // 일어나고, 그러면 노트 <div>들이 실제로 DOM에 그려지지 않는다 — 채보를
  // 다 만든 직후 이 값을 한 번 바꿔서 강제로 리렌더시켜 노트 엘리먼트들을
  // 미리 마운트해둔다.
  ...
  useEffect(() => {
    ...
    const loop = (duration) => {
      ...
      for (const note of chartRef.current) {
        if (!note.judged && now - note.time > missGrace) {
          popNote(note, 'miss');
        }
        if (note.el && !note.el.dataset.gone) {
          const msUntil = (note.time - now) * 1000;
          const progress = 1 - msUntil / difficulty.noteTravelMs; // 0(등장) ~ 1(판정선)
      ...
      if (now > startTimeRef.current + duration + missGrace + 0.5 && !finishedRef.current) {
      ...
      rafRef.current = requestAnimationFrame(() => loop(duration));
    };

    (async () => {
      const startTime = await player.start();
      if (cancelledRef.current) { player.stop(); return; }
      startTimeRef.current = startTime;
      chartRef.current = generateChart(startTime, difficulty);
